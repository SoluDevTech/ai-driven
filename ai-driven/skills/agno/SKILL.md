...
### Pattern: Human-in-the-Loop Tool Confirmation
Gate a sensitive tool behind human approval. Requires a `db` to continue the paused run.
```python
from agno.tools import tool

@tool(requires_confirmation=True)
def delete_records(table: str, count: int) -> str:
    """Delete rows — needs confirmation."""
    return f"Deleted {count} rows from {table}."

agent = Agent(model=..., tools=[delete_records], db=SqliteDb(db_file="tmp/hitl.db"))
run = agent.run("Delete 100 rows from sessions")
if run.is_paused:
    for t in run.tools_requiring_confirmation:
        t.confirmed = True  # or t.confirmed = False; t.confirmation_note = "why"
    run = agent.continue_run(run_id=run.run_id, updated_tools=run.tools)
print(run.content)
```
Async: `arun()` + `acontinue_run()`. Also supports streaming. `requires_confirmation` is mutually exclusive with `requires_user_input` and `external_execution`.
...
- **Extras (verified, agno 2.8.7)**: `pip install sqlalchemy` is required to import `agno.db.sqlite.SqliteDb` (storage/memory), and `pip install fastapi` is required to import anything from `agno.workflow`. Both raise `ModuleNotFoundError` at import time if missing.
